...
package object d {
  ...
  implicit class XD(val x: X) extends AnyVal {
  ...
  implicit class UD(val u: U) extends AnyVal {
  ...
  implicit class ZD(val z: Z) extends AnyVal {
  ...
  object ZD {
    def apply(
      ...
      magB: Vector2,
      gyr: Vector1
    ): Z = Z(VectorD.vertical(dis, vel, accB, magB, gyr))
    ...
    def columns(
      ...
      magB: MatrixD,
      gyr: MatrixD
    ): MatrixD = MatrixD.vertical(dis, vel, accB, magB, gyr)
    ...
    ): MatrixD = MatrixD.horizontal(dis, vel, accB, magB, gyr)
