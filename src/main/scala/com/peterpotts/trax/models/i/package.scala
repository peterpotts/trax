...
package object i {
  ...
  implicit class XI(val x: X) extends AnyVal {
  ...
  object XI {
    def apply(
      ...
      omegaB: Vector3,
      alphaB: Vector3
    ): X = X(VectorD.vertical(d, v, a, r.vector3, omegaB, alphaB))
    ...
    def columns(
      ...
      omegaB: MatrixD,
      alphaB: MatrixD
    ): MatrixD = MatrixD.vertical(d, v, a, r, omegaB, alphaB)
    ...
    ): MatrixD = MatrixD.horizontal(d, v, a, r, omegaB, alphaB)

    val n: Int = 18
  }

  implicit class UI(val u: U) extends AnyVal {
  ...
  implicit class ZI(val z: Z) extends AnyVal {
