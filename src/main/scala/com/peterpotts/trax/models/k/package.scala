...
package object k {
  ...
  implicit class XK(val x: X) extends AnyVal {
    ...
    def e: EulerAngles = EulerAngles.normalize(x(9), x(10), x(11))
    ...
    def -(that: X): X = XK(
      ...
      v = v - that.v,
      a = a - that.a,
      e = (e.unitQuaternion ∘ that.e.unitQuaternion.inverse).eulerAngles,
  ...
  object XK {
    def apply(
      ...
      g: Vector3,
      N: Vector3
    ): X = X(VectorD.vertical(d, v, a, e.vector3, omegaB, alphaB, g, N))
  ...
  implicit class ZK(val z: Z) extends AnyVal {
